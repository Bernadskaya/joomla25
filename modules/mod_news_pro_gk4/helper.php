...
class NSP_GK4_Helper {
	...
	function init($module, $params) {  
		...
		$this->config["ads_profiles"] = $params->get("ads_profiles",'');
	...
	if($this->config['data_source'] == 'ads_profiles')
			{
				$db =& JFactory::getDBO();
				$ids=Array();
				if(!is_array($this->config["ads_profiles"])) $this->config["ads_profiles"]=array($this->config["ads_profiles"]);
				foreach($this->config["ads_profiles"] as $k=>$v)
					{
						$query = 'SELECT * FROM #__bill WHERE profileid='.$v.' AND published=1 AND tend>=now() AND status!=0  ORDER BY ordering ASC';
						$db->setQuery($query);
						$rows=$db->loadObjectList();
						foreach($rows as $row) $ids[]=$row->adid;
						
					}
				
				$this->config['data_source']='k2_articles';
				//$this->config['only_frontpage']=0;
				$this->config['k2_articles']=implode(',',$ids);
			}
